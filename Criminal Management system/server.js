...
app.post("/second", (req, res)=>{
    ...
    connection.query("select * from auth_login where dept_email = ? and passwd = ?",[dept_email,passwd], function(error,results,fields){
        if(results.length>0){
            //res.redirect("/index.html");
            res.redirect("https://apps.powerapps.com/play/e/default-867ebaf4-f8aa-4b55-986e-c961f197a5f3/a/cb000a64-1cb8-40b0-b14c-831b29255dfd?tenantId=867ebaf4-f8aa-4b55-986e-c961f197a5f3&hidenavbar=true");
            //window.location.href("https://apps.powerapps.com/play/e/default-867ebaf4-f8aa-4b55-986e-c961f197a5f3/a/cb000a64-1cb8-40b0-b14c-831b29255dfd?tenantId=867ebaf4-f8aa-4b55-986e-c961f197a5f3&hidenavbar=true");
        } else {
    ...
    //     var sql = "SELECT * FROM login WHERE dept_email=? AND passwd=?";
    //     var values = [
    //         [dept_email, passwd]
    //     ];
    ...
    //         // else
    //         // res.send("Login successfull");
    //             // location.replace("https://apps.powerapps.com/play/e/default-867ebaf4-f8aa-4b55-986e-c961f197a5f3/a/cb000a64-1cb8-40b0-b14c-831b29255dfd?tenantId=867ebaf4-f8aa-4b55-986e-c961f197a5f3&hidenavbar=true");
...
app.post("/contact", (req, res) => {
    ...
    var Weapon_used = req.body.Weapon_used;
        ...
        if(!Victim_Number.length){
            alert("Enter the victim number properly");
            // res.json({'alert' : 'Enter the victim number properly'});
        }else if(Name.length < 3){
            alert("Name must be 3 letters long");
            // res.json({'alert' : 'name must be 3 letters long'});
        }else if(!Age.length){
            alert("Enter the age of victim");
    ...
    connection.connect(function(error){
        ...
        var values = [
            [Victim_Number, Name, Age, Gender, Date_Of_Murder, Address, District, Reason, Position, Weapon_used]
        ];
        connection.query(sql, [values], (error, result)=>{
            if(error) throw error;
            res.redirect("/");
...
app.post('/upload', (req, res) => {
    ...
    // image upload path
    let path = 'public/assets/images/' + imagename;
